...
async function loadData() {
  ...
  try {
    ...
    if (cached) {
      ...
      state.events = (cached.events || []).map(normalizeCachedEvent).filter(Boolean);
      state.rsvps = (cached.rsvps || []).map(normalizeCachedRsvp).filter(Boolean);
      hydrateStats();
      populateCategories();
      render();
      setBanner(`Loaded ${state.events.length} events from events.json.`);
      return;
    }
    ...
    console.error(error);
    setBanner(
      `Could not load data for ${GITHUB_CONFIG.owner}/${GITHUB_CONFIG.repo}. Update GITHUB_CONFIG, make sure the repository is public, and confirm the event and rsvp labels exist.`
    );
...
async function tryLoadEventsJson() {
  try {
    const response = await fetch(github.eventsJsonUrl, { cache: "no-store" });
    if (!response.ok) return null;
    const data = await response.json();
    if (!data || !Array.isArray(data.events) || !Array.isArray(data.rsvps)) return null;
    return data;
  } catch (error) {
...
function normalizeCachedEvent(event) {
  ...
  const start = parseFlexibleDate(event.startDate || event.start || "");
  const end = parseFlexibleDate(event.endDate || event.end || event.startDate || event.start || "") || start;
  if (!event.title || !event.slug || !start) return null;

  return {
    issueNumber: event.issueNumber || null,
    issueCreatedAt: event.issueCreatedAt || new Date().toISOString(),
    issueUrl: event.issueUrl || github.createEventUrl,
    title: event.title,
    slug: slugify(event.slug),
    category: event.category || "Other",
    startDate: event.startDate || event.start || "",
    endDate: event.endDate || event.end || event.startDate || event.start || "",
    start,
    end,
    location: event.location || "Location TBD",
    coverImage: event.coverImage || FALLBACK_IMAGE,
    capacity: event.capacity || "",
    organizer: event.organizer || "Organizer TBD",
    registrationCloses: event.registrationCloses || "",
    tags: Array.isArray(event.tags) ? event.tags : splitTags(event.tags),
...
function normalizeCachedRsvp(rsvp) {
  if (!rsvp) return null;
  const eventId = slugify(rsvp.eventId || rsvp.event_id || "");
  if (!eventId) return null;

  return {
    eventId,
    attendeeName: rsvp.attendeeName || "Guest",
    attendanceStatus: rsvp.attendanceStatus || "Yes, attending",
    guestCount: Number.parseInt(rsvp.guestCount || "1", 10) || 1,
    notes: rsvp.notes || "",
    issueUrl: rsvp.issueUrl || github.rsvpFormUrl
  };
}
